...
<div class="container">
    ...
    <div class="card-body p-4">
        <h2 class="card-title h4 mb-4 text-dark">CSV Previous Loan File Upload & Batch Processing</h2>
...
@script
<script>
    let loanPollingInterval;

    $wire.on('start-loan-polling', () => {
        loanPollingInterval = setInterval(() => {
            $wire.dispatch('check-loan-progress');
        }, 2000); // Check every 2 seconds for batch processing
    });

    $wire.on('stop-loan-polling', () => {
        if (loanPollingInterval) {
            clearInterval(loanPollingInterval);
            loanPollingInterval = null;
        }
    });
